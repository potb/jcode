...
fn test_save_input_for_reload_persists_inflight_queued_continuation() {
    // Must run under a hermetic JCODE_HOME: save_input_for_reload and
    // restore_input_for_reload both resolve their path through the
    // process-global JCODE_HOME, so without the test-env lock a concurrent
    // test can repoint the home between the save and the restore and the
    // restore then finds no file at all.
...
fn test_save_input_for_reload_removes_stale_file_when_state_is_empty() {
    // Same hermetic-home requirement as the in-flight continuation test: this
    // one additionally asserts on the existence of a concrete file under
    // JCODE_HOME, so a concurrent home switch would make the assertions read
