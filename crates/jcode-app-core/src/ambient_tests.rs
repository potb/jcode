...
fn ambient_prompt_ranks_configured_priority_projects_above_busier_ones() {
    ...
    let prioritized = render(
        "project_priority = [\"/home/potb/projects/costo/beakon\", \"/home/potb/jcode\"]\n",
    );
    ...
    let beakon = section
        .find("/home/potb/projects/costo/beakon")
        .expect("beakon listed");
    let jcode = section.find("/home/potb/jcode (").expect("jcode listed");
    assert!(
        beakon < jcode,
        "beakon is the configured first priority, so it must outrank the \
         busier jcode project; got section:\n{section}"
    );
    assert!(
        section.contains("/home/potb/projects/costo/beakon (1 session(s)) [priority]"),
...
fn ambient_prompt_surfaces_a_priority_project_with_no_recent_sessions() {
    ...
    std::fs::write(
        temp.path().join("config.toml"),
        "[ambient]\nenabled = true\nproject_priority = [\"/home/potb/projects/costo/beakon\"]\n",
    )
    .expect("write config");
    crate::config::invalidate_config_cache();
    ...
    let idle = prompt
        ...
        .nth(1)
        .expect("idle section");
    assert!(idle.contains("/home/potb/projects/costo/beakon"));
    assert!(
...
fn ambient_prompt_scopes_the_pr_repo_override_to_its_own_repository() {
    ...
    // push a beakon branch to the jcode fork once it works across projects.
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let prev_home = std::env::var_os("JCODE_HOME");
    crate::env::set_var("JCODE_HOME", temp.path());
