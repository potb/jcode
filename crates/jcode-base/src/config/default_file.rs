...
#
# Migrating from a systemd timer, two differences worth knowing. Last-run
# history lives on disk, so an interval job does not re-fire just because the
# daemon restarted, the way OnUnitActiveSec + Persistent=true behaves. And a
# job inherits the daemon's environment rather than carrying its own
# Environment= lines, so a command that needs a toolchain outside the daemon's
# PATH (a Nix dev shell, a version manager) must invoke it explicitly.
#
# [[cron]]
...
# #                              # NOTE: a prompt is delivered by the ambient
# #                              # pipeline, so it only actually runs when
# #                              # [ambient] enabled = true. With ambient off the
# #                              # fire is queued and waits; cron holds at one
# #                              # outstanding request per job rather than piling
# #                              # up a copy per tick. Exec-mode jobs are
# #                              # unaffected and run either way.
# working_dir = "~/jcode"
